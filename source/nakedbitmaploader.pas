...
type
  ...
  public
    procedure LoadNakedFromStream(stream: TStream; changetransparenttowhite: bool);
    procedure LoadNakedFromBuffer(buffer: Pointer; size: integer);
  end;

implementation

procedure TNakedBitmapLoader.LoadNakedFromStream(stream: TStream; changetransparenttowhite: bool);
var
  ...
  if (pixelformat = pf8bit) then begin //palletized
   //how many entries are there in this palette?
    ...
    for t1 := 0 to header.biClrUsed - 1 do begin
      if changetransparenttowhite and (t1 = 253) then begin
        palette.palPalEntry[t1].peBlue := 255;
        palette.palPalEntry[t1].peGreen := 255;
        palette.palPalEntry[t1].peRed := 255;
